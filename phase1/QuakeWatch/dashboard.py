...
class EarthquakeDashboard:
    ...
    def status():
        ...
        status_info = {
            'service': 'QuakeWatch Flask Application',
            'status': 'running',
            'uptime_seconds': uptime_seconds,
            'uptime_human': f"{uptime_seconds} seconds",
            'started_at_utc': datetime.utcfromtimestamp(APP_START_TIME).strftime('%Y-%m-%d %H:%M:%S UTC'),
            'checked_at_utc': datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')
        }
    ...
    def info():
        app_info = {
            ...
            'description': 'QuakeWatch Flask application containerized and orchestrated for the DevOps Engineer final project',
            'docker_image': 'yonathantoledano/quakewatch:phase1',
